...
MirrorMode::MirrorMode(float density)
{
	grid = new Grid();
	grid->random(density);
}
...
{
	delete grid;
	cout << "MirrorMode Deleted" << endl;
}


//Main method of mirror mode used to run this game mode
void MirrorMode::run(char pause)
{
	...
	while(grid->infinite == true)
	{
		...
		for (int i = 0; i < grid->rows; i++)
		{
			for (int j = 0; j < grid->columns; j++)
			{
				...
				int pop = MirrorMode::checkGrid(i, j);
				//decides fate of box
				MirrorMode::fate(pop, i, j);
			}
...
int MirrorMode::checkGrid(int row, int column)
{
	int count = 0;
	//Right Middle Box
	...
	{
		if (grid->currentArray [row - 1][grid->columns] == 'X')
		{
			count++;
		}
	}
...
void MirrorMode::fate(int pop, int row, int column)
{
	if (pop <= 1)
	{
